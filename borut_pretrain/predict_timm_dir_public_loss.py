...
import os
from pretrain_timm_loss import MyModel
...
def test_model(model, dataloaders):
    ...
    with torch.no_grad():  # Turn off gradients for validation, saves memory and computations
        for images in dataloaders:
            ...
            # Since we expect the model to output a single scalar between 0 and 1,
            # we don't need to apply softmax.
            # Just detach and move the tensor to cpu.
            pred_ = outputs.cpu().detach().numpy().flatten()  # Flatten to ensure it's a 1D array

            # Append the predictions to the prediction array
