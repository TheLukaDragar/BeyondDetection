...
from torch.optim.lr_scheduler import ReduceLROnPlateau,SequentialLR, CosineAnnealingWarmRestarts, LinearLR, CosineAnnealingLR
...
eps = 1e-8
#alha is a list of two values, alpha[0] is the weight of the norm in nrm loss, alpha[1] is the weight of the second loss whish is a variant of the loss connected to rmse
# [1, 0.1] gave best results

# def norm_loss_with_normalization(y_pred, y, alpha=[1, 0.1], p=1, q=2, detach=False, exponent=True):
# params wins uswd in the paper?
def norm_loss_with_normalization(y_pred, y, alpha=[1, 0.1], p=1, q=2, detach=False, exponent=True):
    ...
    if N > 1:  
        ...
        return torch.nn.functional.l1_loss(y_pred, y_pred.detach())  # 0 for batch with single sample.
...
class TimmModel(pl.LightningModule):
    def __init__(
        ...
        self.k = nn.Parameter(torch.ones(1), requires_grad=False)
        ...
        if not pretrained_backbone == None:
            if pretrained_backbone.endswith(".ckpt"):
                ...
                #pretrainec could have different number of output neurons but we dont care
        ...
        classifier_layer = self.backbone.default_cfg["classifier"]

        # Split the string to traverse to the classifier
        parts = classifier_layer.split(".")

        # Traverse to the parent of the classifier layer
        layer = self.backbone
        for part in parts[:-1]:
            layer = getattr(layer, part)

        # Get the number of input features for the last layer
        in_feats = getattr(layer, parts[-1]).in_features

        # Replace the last layer with a new one (with new weights)
        setattr(layer, parts[-1], nn.Linear(in_feats, 512))
        ...
        elif loss == 'norm-in-norm':
            self.loss_fn = self.norm_in_norm
        elif loss == 'norm-in-norm-KL':
            self.loss_fn = self.norm_in_norm_KL
    ...
    def kl_divergence_loss(self, Q, Q_hat):
        ...
        W = torch.nn.functional.softmax(Q, dim=0)
        W_hat = torch.nn.functional.softmax(Q_hat, dim=0)
        
        # Compute the KL divergence
        kl_loss = (W_hat * (W_hat.log() - W.log())).sum()  # Ensure you are summing over the correct dimension
        return kl_loss
    ...
    def forward(self, x):
        ...
        mos = self.backbone(
            x_random_frame
        )  # Output shape: (batch_size, mos)


        #add dropout
        mos = self.fc1(mos)
        
        mos = torch.nn.functional.relu(mos)
        mos = self.drop(mos)
        ...
        #bckbone is deepfake detector that outputs 0 for real and 1 for fake
        #transform to 0 for real and 1 for fake
        # mos = 1 - mos

        # #mos is a value between 1 and 5 so lets scale the output accordingly
        # mos = mos * 4 + 1
    ...
    def linear_mapping(self, pq, sq):
        """Apply linear mapping using least squares regression."""
        # Forming the matrix X by concatenating predictions with a column of ones
        ones = torch.ones_like(pq)
        X = torch.cat((pq, ones), dim=1)

        # Applying least squares regression to find h = [k, b]
        # h = (X^TX)^{-1}X^Tsq
        X_transpose = torch.transpose(X, 0, 1)
        h = torch.linalg.lstsq(X_transpose @ X, X_transpose @ sq).solution

        # Updating self.k and self.b
        self.k = nn.Parameter(h[0].unsqueeze(0), requires_grad=False)
        self.b = nn.Parameter(h[1].unsqueeze(0), requires_grad=False)
        print("k", self.k)
        print("b", self.b)
        ...
        #log k and b
        self.log("k", self.k.item(), on_epoch=True, prog_bar=True, sync_dist=True)
        self.log("b", self.b.item(), on_epoch=True, prog_bar=True, sync_dist=True)


        # # Applying the linear transformation
        # return pq * self.k + self.b
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Reshape y to match the dimensions of preds
        # y_reshaped = y.view(-1, 1)

        #TODO IF STATMENT FOR LOSS TYPE

        # Apply linear mapping with least squares to the predictions
        # mapped_preds = self.linear_mapping(preds, y_reshaped)

        # Compute loss with mapped predictions
        # rmse_loss = self.RMSE(mapped_preds, y_reshaped)
        ...
        # self.log("train_rmse_loss", rmse_loss.item(), on_epoch=True, prog_bar=True, sync_dist=True)
    ...
    def on_train_epoch_end(self):
        ...
        #gather all preds and targets #TODO make DDP work again
        ...
        # Perform linear mapping using the entire training data
        if self.loss_fn == self.norm_in_norm or self.loss_fn == self.norm_in_norm_KL:
            self.linear_mapping(all_preds, all_targets)
        ...
        #log plcc and srcc
        self.log("train_plcc", self.pearson_corr_coef(all_preds.squeeze(), all_targets), on_epoch=True, prog_bar=True, sync_dist=True)
        self.log("train_srcc", self.spearman_corr_coef(all_preds.squeeze(), all_targets), on_epoch=True, prog_bar=True, sync_dist=True)

        if self.loss_fn == self.norm_in_norm or self.loss_fn == self.norm_in_norm_KL:
            all_preds = all_preds * self.k + self.b
            self.log("train_rmse", self.RMSE(all_preds, all_targets), on_epoch=True, prog_bar=True, sync_dist=True)
        else:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # use existing k and b to map preds

        if self.loss_fn == self.norm_in_norm or self.loss_fn == self.norm_in_norm_KL:
            
            mapped_preds = preds * self.k + self.b
        else:
            #for other losses dont map anything
            mapped_preds = preds
    ...
    def norm_in_norm(self, preds, y):
        # print("norm_in_norm")
        # print("preds.shape", preds.shape)
        # print("y.shape", y.shape)
        #adding batch dimension
        return norm_loss_with_normalization(preds, y.unsqueeze(1))
    
    def norm_in_norm_KL(self, preds, y):
        # print("norm_in_norm")
        # print("preds.shape", preds.shape)
        # print("y.shape", y.shape)
        #adding batch dimension
        y=y.unsqueeze(1)
        return norm_loss_with_normalization(preds, y) + self.kl_divergence_loss(preds, y)
    ...
    def configure_optimizers(self):
        ...
        if self.scheduler == "ReduceLROnPlateau":
            ...
            #TODO THIS DOESENT WORK AS EXPECTED!!
            warmup_epochs = self.warmup_epochs  # Define the number of warm-up epochs
            scheduler_warmup = LinearLR(optimizer, start_factor=0.1, total_iters=warmup_epochs)

            # Define the cosine annealing scheduler
            scheduler_cosine = CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=1, eta_min=self.learning_rate * 0.001,verbose=True)
...
if __name__ == "__main__":
    ...
    #scheduler ReduceLROnPlateau or CosineAnnealingWarmRestarts
    parser.add_argument(
        "--scheduler",
        default="ReduceLROnPlateau",
        help="Scheduler to use. Supported values: ReduceLROnPlateau, CosineAnnealingWarmRestarts, CosineAnnealingLR",
    )
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Whether to use auto lr find.",
    )
    ...
    if (args.loss == "norm-in-norm" or args.loss == "norm-in-norm-KL") and args.batch_size < 2:
        raise ValueError("Norm-in-norm loss requires batch size bigger than 1")
    ...
    if args.from_cp_id != "None":
        ...
        #print learning rate
        print("learning rate afer cp set is", model.learning_rate)
    ...
    print("auto_lr_find", args.auto_lr_find)
    if args.auto_lr_find == True:

        #THIS DOESENT WORK YET!
        print("running lr finder")
        #base is 0.00005 which is 5e-5
        lr_finder = Tuner(trainer).lr_find(model, min_lr=1e-8, max_lr=2e-4, train_dataloaders=train_dl, val_dataloaders=val_dl)
    ...
    if trainer.global_rank == 0:
        ...
        best_model = best_model.to("cuda:0") if len(args.devices) == 1 else best_model.to("cuda:1")
        ...
        k,b = model.k, model.b
        k_best,b_best = best_model.k, best_model.b
        print("---")
        print("k", k)
        print("b", b)
        ...
        for stage in stages:
            ...
            with torch.no_grad():
                for x, gt, nameee in ds:
                    ...
                    if args.loss == "norm-in-norm" or args.loss == "norm-in-norm-KL":
                        y = y * k + b
                        y2 = y2 * k_best + b_best
        ...
        # if final score below 81 delete checkpoint
        if sum(scores) / 3 < 0.85:
            print("deleting checkpoint since final score is below 81")

            #find file with wandb_run_id in name
            for file in os.listdir(args.cp_save_dir):
                if wandb_run_id in file:
                    os.remove(os.path.join(args.cp_save_dir, file))
                    print("deleted checkpoint")
                    break


            #delete final model
            timm_models_images_dir = os.path.join(args.final_model_save_dir, wandb_run_id)
            shutil.rmtree(timm_models_images_dir)
            print("deleted final model")
